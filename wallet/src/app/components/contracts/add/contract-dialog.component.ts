...
export class ContractDialogComponent implements OnInit{
    ...
    ngOnInit(){
        if(this.data.title == "LSCX_ABT"){
            ...
            this.description = "ABTs represent digital ownership with respect to an underlying tangible, intangible or financial asset. ModernCash only considers for listing ABTs whose underlying assets are highly fungible, such as commodities and securities. To decide which asset class an ABT represents to we look at its underlying asset. If an ABT is “backed” by a commodity, we will consider it a commodity. Likewise, if an underlying is a security the ABT will undoubtedly be a financial asset.";
        }
        if(this.data.title == "LSCX_MDCS"){
            this.title = 'Crypto Currencies (MDCS)';
            this.img = '~icons/LSCX_MDCS.svg';
