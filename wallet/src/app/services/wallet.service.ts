...
const moderncashPath = homedir+"/.moderncash";
...
export class WalletService {
  ...
  writeAccountLocally(v3, fileName){
    if(!fs.existsSync(moderncashPath)){
      fs.mkdirSync(moderncashPath);
    }
    let filePath =moderncashPath+"/"+fileName+".json";
  ...
  checkFileSaved(v3){
    let files = fs.readdirSync(moderncashPath);
    
    for(let i=0; i<files.length; i++){
      if(files[i].indexOf(v3.address)){
        let data = fs.readFileSync(moderncashPath+"/"+files[i]);
      }
    }
  }
