...
contract ModernCash_MarketStorage{
...
contract ModernCashMarket is SafeMath {
  address public admin; //the admin address
  address public feeAccount; //the account that will receive fees
  address public accountLevelsAddr; //the address of the AccountLevels contract
  address public storageAddr; // the address of the ModernCash_MarketStorage contract
  ...
  function ModernCashMarket(address admin_, address feeAccount_, address accountLevelsAddr_, uint feeMake_, uint feeTake_, uint feeRebate_, uint feeMarket_, address storageAddr_) {
    admin = admin_;
  ...
  function tiker(address _token, string _tokenName, uint _decimals, string _string) payable {
    ...
    ModernCash_MarketStorage(storageAddr).concatTiker(_string);
  ...
  function order(address tokenGet, uint amountGet, address tokenGive, uint amountGive, uint expires, uint nonce, string _string, uint256 price) {
    ...
    ModernCash_MarketStorage(storageAddr).setOrders(tokenGet, tokenGive, _string, price, expires, hash);
  ...
  function trade(address tokenGet, uint amountGet, address tokenGive, uint amountGive, uint expires, uint nonce, address user, uint amount) {
    ...
    if(block.number > expires || safeAdd(orderFills[user][hash], amount) == amountGet){
        ModernCash_MarketStorage(storageAddr).deleteOrders(tokenGet, tokenGive, hash);
    }
